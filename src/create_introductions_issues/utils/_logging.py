...
def get_logger(
    ...
    [`StreamHandler`][logging.StreamHandler]. Next, if the `logfile_path` is provided, log messages will also
    be written to the provided path to a logfile using a [`FileHandler`][logging.FileHandler].
    ...
    * [`WARNING`][logging.WARNING] or [`WARN`][logging.WARN] -  An indication that something unexpected happened, or indicative of some problem in the
...
def format_df_for_logging(
    ...
    """
    Helper function facilitating outputting a Pandas [`DataFrame`][pandas.DataFrame] into a logfile. This function only
